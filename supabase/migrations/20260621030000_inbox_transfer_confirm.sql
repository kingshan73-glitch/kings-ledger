-- 수집함 이체 확정 지원.
-- 이체 문자는 수집 단계에서 guessed_type='transfer'로 저장하고,
-- 사용자가 출금/입금 계좌를 지정한 뒤 hh_transaction(type='transfer')로 확정한다.

alter table public.hh_transaction_inbox
  add column if not exists guessed_from_account_id uuid references public.hh_account(id) on delete set null,
  add column if not exists guessed_to_account_id uuid references public.hh_account(id) on delete set null;

comment on column public.hh_transaction_inbox.guessed_from_account_id is
  '이체 문자 확정용 출금 계좌 추정값';
comment on column public.hh_transaction_inbox.guessed_to_account_id is
  '이체 문자 확정용 입금 계좌 추정값';
...
begin
  ...
  if v_type = 'transfer' then
    ...
    insert into public.hh_transaction
      (owner_auth_uid, txn_date, type, amount, from_account_id, to_account_id, counterparty, source)
    values
      (v_owner, coalesce(r.guessed_date, current_date), 'transfer', r.guessed_amount,
       r.guessed_from_account_id, r.guessed_to_account_id, r.guessed_merchant, 'inbox')
